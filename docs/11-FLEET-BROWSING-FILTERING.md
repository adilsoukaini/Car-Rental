...
> - ✅ **Client-side category + transmission filters and price/name sort** — DONE, but NOT registry-based. `Vehicles/Index.tsx` derives a category select from the loaded vehicles (plus a hardcoded transmission select) and sorts client-side by `daily_rate`/name. This works but is frontend-only and per-page, not the extensible server-side registry this doc specifies.
> - ❌ **Registry-based filtering** — NOT DONE. No `VehicleFilterProvider` interface, no `VehicleFilterRegistry`, no `VehicleCategoryFilter` / `MinDailyRateFilter` / `MaxDailyRateFilter`. Future filter types would still require editing the controller/page rather than registering a provider.
> - ❌ **Registry-based sorting** — NOT DONE. No `VehicleSortOption` interface / `VehicleSortRegistry`; sort is a hardcoded client-side switch.
